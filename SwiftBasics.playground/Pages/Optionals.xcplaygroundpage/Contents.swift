...
if let myUnwrappedPostCode = myPostCode {
    myAddress += myUnwrappedPostCode
} else {
    print("Your postcode is invalid!")
}

guard let myUnwrappedPostCode = myPostCode else {
    fatalError()
}

myAddress += myPostCode!

myAddress += myPostCode ?? "1000"
...
if let myUnwrappedRank = myRank {
    endOfGameSummary = "Congrats! You've achieved rank \(myUnwrappedRank) in this round."
} else {
    endOfGameSummary = "You did not achieve a rank in this round. Better luck next time!"
}

guard let myUnwrappedRank = myRank else {
    "You did not achieve a rank in this round. Better luck next time!"
    fatalError()
}
...
guard let pan = amountOfFoodInsidePan else {
    print("I give up.")
    fatalError()
}
if foodStock > 0 {
    if let stove = cookingMode {
        switch stove {
        case "High": cookingMode = "Medium"
        case "Medium": cookingMode = "Low"
        case "Low": cookingMode = nil
        default : cookingMode = nil
        }
        foodStock-=3
